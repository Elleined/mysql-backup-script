...
rem ***backup creation***
docker exec ${MYSQL_HOST_NAME_OR_IP_ADDRESS} /usr/bin/mysqldump --user=%mysql_user% --password=%mysql_password% --all-databases --routines --triggers --events --result-file="%backup_path%\%backup_name%.sql"
if %ERRORLEVEL% neq 0 (
